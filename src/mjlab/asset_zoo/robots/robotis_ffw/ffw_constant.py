...
# Compute action scale from XML actuator control ranges
FFW_ACTION_SCALE: dict[str, float] = {}
_spec = get_spec()
for actuator in _spec.actuators:
  # Extract joint name from actuator name (format: "XXX_actuator_JOINT_NAME")
  match = re.search(r'actuator_(.+)$', actuator.name)
  if match and actuator.ctrllimited and len(actuator.ctrlrange) == 2:
    joint_name = match.group(1)
    ctrl_min, ctrl_max = actuator.ctrlrange
    # Scale is half the control range (normalized to [-1, 1] action space)
    FFW_ACTION_SCALE[joint_name] = (ctrl_max - ctrl_min) / 2.0
